...
public class InvoiceLine
{
	...
	public InvoiceLine(int quantity, int articleId)
	{
		...
		this.price = quantity * Article.getArticleById(articleId).getPrice();
	}
	...
	public InvoiceLine(Lot lot)
	{
		...
		this.price = lot.quantity * Article.getArticleById(lot.articleId).getPrice() * (1 - (lot.priceReductionPercentage / 100));
	}
